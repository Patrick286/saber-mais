...
export default {
  ...
  methods: {
    carregarFlashcards() {
  axios.get('http://18.225.32.96:8080/api/flashcards')
    .then(response => {
    ...
    checkSimuladoStatus() {
      ...
      if (userEmail) {
        axios.get(`http://18.225.32.96:8080/api/user/by-email?email=${userEmail}`)
          .then((response) => {
...
loadNextFlashcard() {
  ...
  if (userEmail) {
    ...
    if (this.userActivities.simuladosUmRealizado === 1) {
      ...
      axios.get(`http://18.225.32.96:8080/api/flashcards/?id=${flashcardId}`)
        .then((response) => {
  ...
  if (userEmail && key) {
    ...
    axios.put(`http://18.225.32.96:8080/api/user/updateField?email=${userEmail}`, {
      chave: key,
    ...
    try {
      const response = await axios.get(`http://18.225.32.96:8080/api/user/by-email?email=${userEmail}`);
  ...
  async startSimuladoDois() {
    ...
    async endSimulado() {
      ...
      try {
        ...
        await axios.put(`http://18.225.32.96:8080/api/user/updateField?email=${userEmail}`, {
  ...
  const userEmail = localStorage.getItem('email'); // Pegue o email do localStorage
  if (userEmail) {
    axios.get(`http://18.225.32.96:8080/api/user/by-email?email=${userEmail}`) // Faz a requisição GET para buscar as atividades do usuário
      .then((response) => {
    ...
    fetchQuestions() {
      axios.get(`http://18.225.32.96:8080/api/questions`) // Faz a requisição GET para o endpoint
        .then(response => {
